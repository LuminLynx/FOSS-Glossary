    ...
    terms: [
      {
        slug: 'e2e-test-term',
        term: 'E2E Test Term',
        definition: 'This is a test term created specifically to validate the end-to-end pipeline flow from merge to deployment with proper version tracking.',
      ...
      },
      {
        slug: 'another-term',
        term: 'Another Term',
        definition: 'A second term to ensure the pipeline handles multiple terms correctly and generates proper statistics and metadata.',
        tags: ['example']
      }
    ]
  };
...
test('E2E: complete pipeline from merge to deployment', () => {
  ...
  assert.equal(
    ...
    true,
    'Validation should pass for valid test terms'
  );
  assert.match(
  ...
  assert.ok(
    results.termsJson,
    'terms.json should be created'
  );
  assert.ok(
    results.termsJson.version,
    'terms.json should contain version metadata'
  );
  assert.ok(
    results.termsJson.generated_at,
    'terms.json should contain generated_at timestamp'
  );
  assert.equal(
    results.termsJson.terms_count,
    2,
    'terms.json should contain correct terms_count'
  );
  assert.equal(
    results.termsJson.terms.length,
    2,
    'terms.json should contain all terms'
  );
  ...
  assert.equal(
    results.landing.success,
    true,
    'Landing page generation should succeed'
  );
  ...
  const versionMatch = results.indexHtml.match(/window\.__TERMS_JSON_URL\s*=\s*['"]\.\/terms\.json\?ver=([^'"]+)['"]/);
  assert.ok(
    versionMatch,
    'Should be able to extract version from TERMS_JSON_URL'
  );
  ...
  assert.deepEqual(
    termSlugs,
    sortedSlugs,
    'Terms in JSON should be sorted alphabetically by slug'
  );
...
test('E2E: export --only-if-new flag works correctly', () => {
    ...
    terms: [
      {
        slug: 'initial-term',
        term: 'Initial Term',
        definition: 'This is the initial term that exists from the beginning of the repository history.'
      }
    ]
  };
  ...
  try {
    ...
    const firstExport = spawnSync('node', [EXPORT_SCRIPT, '--out', tmpTermsJsonPath, '--only-if-new'], {
      cwd: tmpDir,
      encoding: 'utf8',
      stdio: 'pipe'
    });
    ...
    const secondExport = spawnSync('node', [EXPORT_SCRIPT, '--out', tmpTermsJsonPath, '--only-if-new'], {
      cwd: tmpDir,
      encoding: 'utf8',
      stdio: 'pipe'
    });
    ...
    assert.equal(
      secondExport.status,
      0,
      'Export should exit successfully even when skipping'
    );
    assert.match(
      secondExport.stdout,
    ...
    const updatedTerms = {
      terms: [
        ...initialTerms.terms,
        {
          slug: 'new-term',
          term: 'New Term',
          definition: 'This is a brand new term added to test the --only-if-new functionality in the export script.'
        }
      ]
    };
    ...
    const thirdExport = spawnSync('node', [EXPORT_SCRIPT, '--out', tmpTermsJsonPath, '--only-if-new'], {
      cwd: tmpDir,
      encoding: 'utf8',
      stdio: 'pipe'
    });
    ...
    assert.equal(
      thirdExport.status,
      0,
      'Export should succeed with new terms'
    );
    assert.match(
      thirdExport.stdout,
      /Wrote|✅/,
      'Should show success message for new terms'
    );
    
    // Verify the exported file exists and has correct count
    assert.ok(
      fs.existsSync(tmpTermsJsonPath),
      'terms.json should be created'
    );
    
    const exportedData = JSON.parse(fs.readFileSync(tmpTermsJsonPath, 'utf8'));
    assert.equal(
      exportedData.terms_count,
      2,
      'Exported JSON should contain both terms'
    );
  } finally {
    fs.rmSync(tmpDir, { recursive: true, force: true });
  }
