...
test('scoreTerm: achievement badges based on score', () => {
  ...
  assert.ok(
    perfectResult.badges.includes('💯 Perfectionist'),
    'Score 90+ should get Perfectionist badge'
  );
  ...
  assert.ok(
    starResult.badges.includes('⭐ Star Contributor'),
    'Score 80-89 should get Star Contributor badge'
  );

  const strongResult = scoreTerm(strongTerm);
  assert.ok(
    strongResult.badges.includes('💪 Strong Entry'),
    'Score 70-79 should get Strong Entry badge'
  );
});
